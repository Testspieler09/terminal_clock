...
impl QuotesConfig {
    ...
    pub fn into_quotes_with_initial(self) -> Result<(Vec<Quote>, Option<u16>), String> {
        ...
        let mut initial_idx: Option<u16> = None;
        ...
        for (i, q) in self.quote.into_iter().enumerate() {
            ...
            if q.initial {
                initial_idx = Some(quotes.len().try_into().unwrap_or(0));
            }
...
impl QuoteLoader {
    fn load_user_quotes(config_path: &Path) -> LoaderResult<(Vec<Quote>, Option<u16>)> {
    ...
    pub fn load_quotes(config_path: &Path) -> LoaderResult<(Vec<Quote>, Option<u16>)> {
        ...
        let user_initial = match Self::load_user_quotes(config_path) {
            Ok((user_quotes, idx)) => {
                let offset: u16 = quotes.len().try_into().unwrap_or(0);
